				...
				<h2 class="content-header-title float-left mb-0">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">
							...
							?>
							<a href="{{url('manage-purchaseReturnReport.html')}}" title="<?php echo $pageTitle;?>"><?php echo $pageTitle;?></a>
						</li>
					</ol>
				</h2>
			</div>
										...
										<tbody class="tb-data">
											@if ($resultData->count() > 0 )
												...
												<tr>
													...
													<td class="text-center" > <a target="_blank" title="Purchase Order" href="{{url('manage-PurchaseReturn.html/edit/'.$row->purchase_return_id)}}"> {{$row->reference_no}} </a></td>
													<td>{{ucfirst($row->warehouse_name)}}</td>
													<td>{{ucfirst($row->supplier_name)}}</td>
													<td class="pl-1">
														@foreach($purchase_items as $value)
															@if($value->purchase_return_id == $row->purchase_return_id)
